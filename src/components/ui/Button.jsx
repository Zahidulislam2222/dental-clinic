...
const Button = ({
  ...
  const baseClasses = `font-heading font-semibold rounded-xl transition-all duration-300 inline-flex items-center justify-center gap-2 ${variants[variant]} ${sizes[size]} ${pulse ? 'animate-pulse-teal' : ''} ${disabled ? 'opacity-50 cursor-not-allowed' : ''} ${className}`;
  ...
  return (
    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className={baseClasses} disabled={disabled || loading} {...props}>
      {content}
    </motion.button>
  );
};
